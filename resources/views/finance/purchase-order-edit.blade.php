<x-app-layout>
    <x-page-header title="Edit {{ $po->po_number }}" :back="route('finance.po.show', $po->id)">
    </x-page-header>

    <div class="max-w-4xl mx-auto space-y-4" x-data="poForm({
        products: {{ Js::from($productMap) }},
        existingItems: {{ Js::from($existingItems) }},
        ...
        <form method="POST" action="{{ route('finance.po.update', $po->id) }}">
            @csrf
            @method('PUT')
            <!-- PO Details -->
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                        ...
                        <input type="text" value="{{ $po->po_number }}" disabled class="block w-full rounded-lg bg-gray-50 dark:bg-gray-700/50 border-gray-300 dark:border-gray-600 dark:text-gray-400 text-sm font-mono">
                    </div>
                    <div>
                        ...
                        <select name="vendor_id" required x-model="vendorId" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                            ...
                            <option value="{{ $vendor->id }}" @selected($po->vendor_id === $vendor->id)>{{ $vendor->brand_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Order Date *</label>
                        <input type="date" name="order_date" required value="{{ $po->order_date->format('Y-m-d') }}" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Expected Delivery</label>
                        <input type="date" name="expected_delivery_date" value="{{ $po->expected_delivery_date?->format('Y-m-d') }}" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                    </div>
                    <div>
                        ...
                        <select name="status" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                            @foreach(['draft','submitted','confirmed'] as $st)
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
                    <div>
                        ...
                        <input type="text" name="payment_method" value="{{ $po->payment_method ?? '' }}" placeholder="Wire, CC, Net 30..." class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Payment Terms</label>
                        <input type="text" name="payment_terms" value="{{ $po->payment_terms ?? '' }}" placeholder="Net 30, Net 60..." class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Amount Paid</label>
                        <input type="number" name="amount_paid" step="0.01" min="0" value="{{ $po->amount_paid }}" x-model.number="amountPaid" @input="recalc()" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Shipping Cost</label>
                        <input type="number" name="shipping_cost" step="0.01" min="0" value="{{ $po->shipping_cost }}" x-model.number="shippingCost" @input="recalc()" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Tax Amount</label>
                        <input type="number" name="tax_amount" step="0.01" min="0" value="{{ $po->tax_amount }}" x-model.number="taxAmount" @input="recalc()" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Discount</label>
                        <input type="number" name="discount_amount" step="0.01" min="0" value="{{ $po->discount_amount }}" x-model.number="discountAmount" @input="recalc()" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">
                    </div>
                </div>
                <div class="mt-3">
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Notes</label>
                    <textarea name="notes" rows="2" class="block w-full rounded-lg border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 text-sm">{{ $po->notes ?? '' }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-2">
                <a href="{{ route('finance.po.show', $po->id) }}" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">Cancel</a>
    ...
    function poForm(data) {
        let nextId = 0;
        const existingItems = (data.existingItems || []).map(item => {
        ...
        return {
            ...
            items: existingItems.length ? existingItems : [{ id: nextId++, product_id: '', product_name: '', asin: '', upc: '', qty: 1, unit_cost: 0, unit_shipping: 0, unit_labeling: 0, unit_other_costs: 0 }],
            addItem() {
                this.items.push({ id: nextId++, product_id: '', product_name: '', asin: '', upc: '', qty: 1, unit_cost: 0, unit_shipping: 0, unit_labeling: 0, unit_other_costs: 0 });
                this.recalc();
            },
            autofillItem(index) {
