<x-app-layout>
    @php
    $items = \App\Models\EmailQueue::with('vendor')->where('status', 'failed')->latest()->paginate(25);
    @endphp

    <x-page-header title="Failed Emails" :back="route('dashboard')" :count="$items->total() . ' total'" />
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $item->attempts }} / {{ $item->max_attempts }}</td>
                        <td class="px-4 py-3 text-sm text-red-500 max-w-xs truncate">{{ $item->last_error }}</td>
                    </tr>
                    @empty
                    <tr><td colspan="5" class="px-4 py-8">
                        <x-empty-state icon="mail" title="No failed emails" description="Good news! No failed email deliveries." />
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
